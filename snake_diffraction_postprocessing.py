...
def plot_snake_velocity(
    plot_params: dict,
    period,
    filename="snake_diffraction_velocity_test6.png",
    SAVE_FIGURE=False,
):
    ...
    ax.plot(time_per_period[:], velocity_in_rod_roll_dir[:, 0], c=to_rgb("xkcd:bluish"), label="lateral")
...
def plot_video(
        plot_params: dict,
        video_name='snake_diffraction_test6.gif',
        fps=60,
        xlim=(0, 4),
        ylim=(-1, 1),
    ...
    com_positions_over_time = np.array(plot_params["center_of_mass"])

    print("plot video")
    # FFMpegWriter = manimation.writers["ffmpeg"]
    # Choose the ImageMagick writer
    PillowWriter = manimation.writers["pillow"]
    metadata = dict(title="Movie Test", artist="Matplotlib", comment="Movie support!")
    # writer = FFMpegWriter(fps=fps, metadata=metadata)
    # writer = ImageMagickWriter(fps=fps, metadata=metadata)
    ...
    patch1_A_coord = np.array([0.6,  0.15])
    patch1_B_coord = np.array([0.9, 0.15])
    patch1_C_coord = np.array([0.9,  -0.15])
    patch1_D_coord = np.array([0.6,  -0.15])
    ...
    # Add the patch as a transparent pink rectangle
    patch = patches.Polygon(
        patch_vertices, closed=True, edgecolor='black', facecolor='purple', alpha=0.15
    )
    ax.add_patch(patch)


    rod_lines_2d = ax.plot(positions_over_time[0][2], positions_over_time[0][0],'k')[0]
    com_lines_2d = ax.plot([], [], 'r')[0]
    # plt.axis("equal")
    with writer.saving(fig, video_name, dpi=150):
        for time in tqdm(range(1, len(plot_params["time"]))):
            ...
            com_lines_2d.set_xdata(com_positions_over_time[:time, 2])
            com_lines_2d.set_ydata(com_positions_over_time[:time, 0])
...
def plot_curvature(
        ...
        period,
        save_fig=False,
        filename="snake_diffraction_curvature2",
):
