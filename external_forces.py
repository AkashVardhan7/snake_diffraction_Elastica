...
class LiftingTorques(NoForces):
    """ This class applies Lifting torques along the body. The applied muscle torques are treated as applied external forces.
    This class can apply muscle torques as a traveling wave with a beta spline or only as a traveling wave.
    ...
    def __init__(
        ...
        if with_spline:
            ...
            def constant_function(input):
                """ Return array of ones same as the size of the input array.
                This function is called when Beta spline function is not used.
    ...
    def compute_lifting_torques(
        ...
        torque_mag = np.maximum(0,(factor* my_spline* Amplitude* np.cos(angular_frequency * time + kt * s + phase_shift)+ 1 ),)
